...
class RewardModelScore:
    ...
    def r_hat(self, x):
        ...
        r_raw = np.mean(r_hats) # [-1, 1]
        return r_raw
        # r_scaled = r_raw * 0.5 + 0.5 # [0, 1]
        # return r_scaled * 10 # [0, 10]
    ...
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1=None, img_t_2=None):
        ...
        if self.vlm_label:
            ...
            for idx, (img1, img2) in enumerate(zip(img_t_1, img_t_2)):
                ...
                img1_pil.save(image_save_path) # for scores, we only query 1 image
                gpt_image_paths.append(image_save_path)

                diff = np.linalg.norm(img1 - img2)
                if diff < 1e-3: # only 10 pixel has 1 diff
