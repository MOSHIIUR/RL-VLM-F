...
class RopeConfigurationEnv(RopeFlattenEnv):
    ...
    def _reset(self):
        ...
        if hasattr(self, 'action_tool'):
            ...
            self.action_tool.set_one_picker_pos(endpoint1_picker_pos, index=0)
    ...
    def generate_alphabet_positions(self):
        ...
        print('character_loc_path ',character_loc_path)
    ...
    def compute_reward(self, action=None, obs=None, **kwargs):
        ...
        current_pos[:, 0] -= np.mean(current_pos[:, 0])
        current_pos[:, 2] -= np.mean(current_pos[:, 2])
...
if __name__ == '__main__':
    ...
    env = RopeConfigurationEnv(cached_states_path='rope_configuration_1_init_states.pkl', **env_dict)
