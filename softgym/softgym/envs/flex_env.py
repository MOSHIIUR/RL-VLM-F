...
class FlexEnv(gym.Env):
    ...
    def reset(self, config=None, initial_state=None, config_id=None):
        ...
        if config is None:
            if config_id is None:
                ...
                #     print('this branch config ', config_id)
                # else:
                ...
                #     print('this branch config ', config_id)
                config_id = np.random.randint(low=0, high=len(self.cached_configs)) if not self.deterministic else 0
    ...
    def render(self, mode='rgb_array', hide_picker=False):
        if mode == 'rgb_array':
            # if hide_picker:
            #     import pdb;pdb.set_trace()
            #     old_shape_states = pyflex.get_shape_states()
            #     new_states = old_shape_states.copy().reshape(-1, 14)
            #     new_states[:, :3] = [0, -1, 0]
            #     new_states[:, 7:10] = [0, -1, 0]
            #     # pyflex.set_shape_states(new_states.flatten())
                # pyflex.step()
            ...
            # if hide_picker:
            #     pyflex.set_shape_states(old_shape_states)
