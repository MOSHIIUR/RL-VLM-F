...
from softgym.softgym.action_space.action_space import  Picker, PickerPickPlace, PickerQPG, PickerPlace
...
class ClothEnv(FlexEnv):
    def __init__(self, observation_mode, action_mode, num_picker=2, render_mode='particle', picker_radius=0.05, picker_threshold=0.005, particle_radius=0.00625, **kwargs):
        ...
        assert action_mode in ['picker', 'pickerpickplace', 'sawyer', 'franka', 'picker_qpg', "pickerplace"]
        ...
        elif action_mode == 'pickerplace':
            self.action_tool = PickerPlace(num_picker=num_picker, particle_radius=particle_radius, env=self, picker_threshold=picker_threshold,
                                               picker_low=(-0.15, -0.15), picker_high=(0.15, 0.15), picker_radius=picker_radius)
            self.action_space = self.action_tool.action_space
            assert self.action_repeat == 1
    ...
    def _get_obs(self):
        ...
        if self.action_mode in ['sphere', 'picker', 'pickerplace']:
